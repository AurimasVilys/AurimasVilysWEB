...
def rateMovie( movieId ):
        ...
        if(setRating == 10) or (re.search('^[0-9](\.[0-9]*)?$', setRating)):
                if (rated[0]["Rating"] == "Not Rated"):
                        rated[0]["Rating"] = setRating
                else:
                        rating = rated[0]["Rating"]
                        rated[0]["Rating"] = (float(0 if rating == "Not Rated" else rating) + (0 if setRating == "Not Rated" else float(setRating))) / 2
                return jsonify(rated[0]), 200
        else:
                return jsonify({'Error':'Rating has to be between 0-10'}), 404
